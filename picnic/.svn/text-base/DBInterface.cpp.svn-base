...
std::string DBInterface::processColumns(sqlite3_stmt* statement, int columns)
{
    std::string output = "{";
    int type;
    std::string cell;
...
std::string DBInterface::query(std::string query, JsonMode mode)
{
    ...
    if(step == SQLITE_ROW)
    {
        ...
        output += processColumns(statement, columns);
        ...
        {
            if(mode == ALL) output += "," + processColumns(statement, columns);
