...
#include "picnic2/PicnicServer.h"
...
public:
    MasterBot(NetworkFactories* networkFactories) {
		...
		server = new PicnicServer("picnic2/picnic.schema", "picnic2.db", muc_manager);
...
void daemonise() {
    ...
    // Set the current working directory to the build directory
    if (chdir("/home/ec2-user/builds/SwiftClient/") == -1) {
        _exit(0);
